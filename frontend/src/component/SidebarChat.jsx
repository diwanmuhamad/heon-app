...
const SidebarChat = ({openSide, setOpenSide, setChatData}) => {
        ...
        <div className={`sm:w-[20%] h-screen p-3 bg-zinc-900 sm:flex sm:flex-col ${openSide?"flex flex-col absolute w-[200px] sm:static z-10":"hidden"}`}>
            <div className="h-1/6 w-full">
                <div className="h-2/4 border-[1px] border-white rounded-lg w-full">
                    <button 
                    onClick={()=>{setChatData([]); setOpenSide(false)}}
                    className="appearance-none h-full w-full text-white hover:bg-stone-800 hover:rounded-lg">+ New chat</button>
                </div>
            </div>
